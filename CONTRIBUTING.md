...
See [manual](https://github.com/error-ident/go-knc-admin/projects/3) for more information.
...
For now, you need to add your fork as a remote on the original **\$GOPATH**/src/go-knc-admin clone, so:

```bash

$ go get go-knc-admin
$ cd $GOPATH/src/go-knc-admin # GOPATH is $HOME/go by default.
...
Notice: `go get` return `package go-knc-admin: no Go files in /go/src/go-knc-admin` is normal.
...
* [Tasks](https://github.com/error-ident/go-knc-admin/projects)
